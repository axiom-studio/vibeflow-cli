...
func TestReviewTUIBinaryConsent(t *testing.T) {
	...
	var registrations, heartbeats, stopped, projects, repositories atomic.Int64
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == "POST" && strings.HasSuffix(r.URL.Path, "/pr-review-runners"):
			...
			if r.URL.Path != "/rest/v1/vibeflow/projects/66/pr-review-runners" || body["provider"] != "github" || body["repository_link_id"] != float64(7) {
	...
	for _, killOwner := range []bool{false, true} {
		t.Run(fmt.Sprintf("accept_kill_%t", killOwner), func(t *testing.T) {
			...
			if registrations.Load() != before+1 || heartbeats.Load() <= beats || repositories.Load() <= links || projects.Load() <= lookups {
			...
			for stopped.Load() != before+1 && time.Now().Before(deadline) {
				time.Sleep(20 * time.Millisecond)
			}
			if stopped.Load() != before+1 {
