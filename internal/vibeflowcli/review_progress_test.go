...
func TestReviewProgressReportsCheckoutThenCompletion(t *testing.T) {
	...
	case <-completed:
	case err := <-done:
		t.Fatalf("execution ended before completion progress: %v", err)
	case <-time.After(10 * time.Second):
		t.Fatal("completion progress was not renewed promptly")
	}
	if err := <-done; err != nil {
		t.Fatal(err)
	}
